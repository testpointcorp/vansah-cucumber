...
function processScenario(scenario, featureName, options) {
  ...
  return {
    ...
    duration: calculateTotalDuration(scenario.steps),
    failedStep: findFailedStepIndex(scenario.steps),
    timestamp: scenario.start_timestamp,
    steps: options.stepLevel ? scenario.steps : null
  };
}
